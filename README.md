...
* **データ入力:**
  * 初回表示時にはデザイン確認用のサンプルデータが入力されており、すぐにスライドプレビューを確認できます。
  * **タイトルスライド:** 氏名、氏名（ふりがな）、職種、会社名、紹介文を入力できます。
  * **職務経歴:** 職務経歴のエントリーを動的に追加・管理できます。
  * **プロジェクト:** 期間、役割、概要、担当内容、成果、技術スタックを動的に追加・管理できます。
  * **技術スキル:**
    * スキルを「主力」「実務経験」「基礎学習」の3区分で整理できます。
  ...
  * **強みと展望:**
    ...
    * 資格を追加・管理できます。
* **データ管理:**
  * **JSON編集:** JSONの貼り付け、JSONファイルの読み込み、現在のデータのダウンロードができます。
  * **部分強調:** JSON内で `**強調したい箇所**` と記述すると、資料上ではその部分だけが赤色の太字で表示されます。
  * **プロンプトコピー:** ポートフォリオJSONを生成AIに作成してもらうためのプロンプトをコピーできます。
* **ユーザーインターフェース:**
...
* **プレビュー:**
  * 生成されたマイポートフォリオドキュメントを同じタブでプレビューし、エディタに戻れます。
  
### マイポートフォリオ プレビューア (`preview.html`)

* **エディタへ戻る:**
  * プレビュー前の入力状態を保ったままエディタへ戻れます。
* **ビジュアライゼーション:**
...
1. ローカルWebサーバーを起動します（例: `python -m http.server 8000`）。
2. **Webブラウザで `http://localhost:8000/editor.html` を開く:** マイポートフォリオエディタが起動します。
3. **エディタを使用してマイポートフォリオデータを作成します。**
4. **データをJSONファイルとして保存します。**
5. エディタの「プレビュー」からプレビューアを開きます。
6. **プレビューをPDFとして保存します。**

保存データは `schemaVersion: 2` の経歴データJSONです。プレビューのHandlebarsテンプレートは
`templates/preview/*.html` にあり、表示都合の設定をJSONへ持たせない構成です。

## デバッグモードの切り替え

まずリポジトリ直下にある `*.env.example` を `.env` にコピーします。
`.env` で `DEBUG_MODE` の値を変更すると、コンソール出力の有無を切り替えられます。
値を `true` にするとログが有効になり、`false` で無効になります。
ファイル変更後はページを再読み込みしてください。
