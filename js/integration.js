// integration.js - 画像アップロード機能を統合するためのスクリプト

// プロジェクトに新しいJavaScriptファイルを追加します。
// このスクリプトはeditor.htmlとpreview.htmlの両方に追加されます。

// DOMContentLoadedイベントが発生したときの処理
document.addEventListener('DOMContentLoaded', function() {
    console.log('画像アップロード機能の統合を開始します');
    
    // 必要なHandlebarsヘルパーを登録（テンプレートで使用）
    ...
    if (!window.location.pathname.includes('preview.html')) {
      // upgradeExistingProjectsが定義されるまで待機してから実行
      const checkAndExecute = function() {
        if (typeof upgradeExistingProjects === 'function') {
          console.log('既存のプロジェクトを画像アップロード対応にアップグレードします');
          upgradeExistingProjects();
        } else {
          console.log('upgradeExistingProjects関数がまだ読み込まれていません、再試行します');
  ...
  function setupImageUploader(projectContainer) {
    ...
    if (illustrationPathInput.value) {
      if (illustrationPathInput.value.startsWith('data:image/')) {
        // Base64データの場合
        ...
        // ファイルアップロードタブをアクティブに
        showTab('file');
      } else if (illustrationPathInput.value.startsWith('http')) {
        // URLの場合
        previewImg.src = illustrationPathInput.value;
        illustrationPreview.style.display = 'block';
    ...
    previewImg.addEventListener('error', function() {
      ...
      if (urlInputTab.classList.contains('active') && this.getAttribute('src') && this.getAttribute('src') !== '') {
        alert('画像の読み込みに失敗しました。URLが正しいか確認してください。');
        illustrationPreview.style.display = 'none';
      }
    });
  ...
  window.upgradeExistingProjects = upgradeExistingProjects;